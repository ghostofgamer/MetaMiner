...
public class CardInventoryView : MonoBehaviour
{
    ...
    [SerializeField]
    private TextMeshProUGUI availableEnergyText;

    [SerializeField]
    private Image itemPictureImage;

    [SerializeField]
    private Slider availableEnergySlider;
    ...
    public void ShowCard(CardState state)
    {
        ...
        upgradesText.text = $"{state.ElectricityLevel + state.PowerLevel + state.BatteryLevel}/15";
        ...
        availableEnergyText.text = $"{state.EnergyAvailable}/{batteryConfig.BatteryLimit}";
        availableEnergySlider.minValue = 0;
        availableEnergySlider.maxValue = batteryConfig.BatteryLimit;

        availableEnergySlider.value = state.EnergyAvailable;
    }
}
